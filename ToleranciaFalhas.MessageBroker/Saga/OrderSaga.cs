...
{
    public class OrderSaga : Saga<OrderSagaState, PaymentStatus, Guid>
    {
            ...
            : base(serviceScopeFactory)
        {
            Configure(builder =>
            {
                builder.When<NewOrderEvent>(PaymentStatus.Pending)
                    .TransitionTo(PaymentStatus.AwaitingPayment)
                    .ThenExecute((serviceProvider, state) => NotifyPaymentService(serviceProvider, state));

                builder.When<PaymentApprovedEvent>(PaymentStatus.AwaitingPayment)
                    .TransitionTo(PaymentStatus.Paid)
                    .ThenExecute((serviceProvider, state) => NotifyPaymentApproved(serviceProvider, state));

                builder.When<PaymentRejectedEvent>(PaymentStatus.AwaitingPayment)
                    .TransitionTo(PaymentStatus.PaymentRejected)
